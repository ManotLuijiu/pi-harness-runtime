...
import { buildLoopTaskGraph } from "./loop-types.js";
import type { SharedBlackboard } from "./blackboard.js";

function encodeNextAction(la: {
	taskId: string;
	agentType: string;
	iteration: number;
	prompt?: string;
...
async function monitorLoop(
	...
	while (true) {
		...
		if (reportNode?.status === "done") {
			...
			const summary =
				verdict !== "unknown" ? `Verdict: ${verdict}` : "Loop completed";

			const reportPath = join(
				paths.reviews,
				`loop-${loopId.slice(0, 8)}-final.md`,
			);
			writeFileSync(
				reportPath,
				`# Loop Result — ${loopId.slice(0, 8)}
...
async function runLoop(args: string[]): Promise<void> {
	...
	const prompt =
		(args.find((a) => a.startsWith("--prompt=")) ?? "")
			.slice("--prompt=".length)
			.replace(/^["']|["']$/g, "") || "No prompt";
	...
	writeFileSync(
		join(paths.root, `loop-${loopId}.config.json`),
...
if (!args.length || args[0] === "--help" || args[0] === "-h") {
	console.log(
		'Usage: bun harness/loop-controller.ts <write_count> <review_count> --prompt="..."\nExample: bun harness/loop-controller.ts 3 2 --prompt="build REST API"',
	);
	process.exit(0);
}
