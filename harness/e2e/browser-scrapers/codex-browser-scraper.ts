 ...
 */
export class CodexBrowserScraper extends BaseBrowserScraper<CodexQuotaData> {
	...
	protected async extractData(): Promise<CodexQuotaData | null> {
	...
	private async extractUsageData(): Promise<Omit<CodexQuotaData, "provider" | "scrapedAt"> | null> {
		if (!this.page) return null;
	...
	private async extractAdditionalData(): Promise<Partial<CodexQuotaData>> {
		const result: Partial<CodexQuotaData> = {};
	...
	private async extractFromHeadings(): Promise<Omit<CodexQuotaData, "provider" | "scrapedAt"> | null> {
		if (!this.page) return null;
	...
	private async extractChartData(): Promise<Omit<CodexQuotaData, "provider" | "scrapedAt"> | null> {
		if (!this.page) return null;
		...
		if (charts.length > 0) {
			...
			const foundResult: { percent: number } | null = null;
			for (const canvas of canvases) {
				const parent = canvas.parentElement;
				if (parent) {
					const attrNames: string[] = Array.from(parent.getAttributeNames());
 ...
 */
export async function scrapeCodexQuota(): Promise<CodexQuotaData | null> {
