...
interface QuotaFile {
	miniMax?: {
		fiveHour?: { percent?: number; resetAt?: string };
		weekly?: { percent?: number; resetAt?: string };
	};
	updatedAt?: string;
}
...
export function isPiLensAvailable(): boolean {
	return (
		Boolean(process.env.PI_LENS_PID) ||
...
function fmtResetAt(iso?: string): string {
	...
	try {
		...
		if (hours > 0) return ` (resets in ${hours}h ${remMins}m)`;
...
export class StatusLineManager {
	...
	update(data: { loopStatus?: string; quota?: QuotaData }): void {
		if (data.loopStatus !== undefined) this.loopStatus = data.loopStatus;
		if (data.quota) this.lastQuota = data.quota;
		this._writeStatusFile();
	}
	...
	private _pollQuota(): void {
		try {
			if (!existsSync(this.quotaFile)) return;
	...
	private _writeStatusFile(): void {
		if (isPiLensAvailable()) return; // pi-lens handles its own display
 ...
 */
export function writeQuotaFile(data: QuotaData): void {
	const dir = join(homedir(), ".pi-harness");
	const file = join(dir, "quota.json");
	try {
		if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
		const payload: QuotaFile = {
			miniMax: {
				fiveHour: {
