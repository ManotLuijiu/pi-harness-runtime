...
function minimaxDefaults() {
    ...
    if (!apiKey) {
        throw new Error("MINIMAX_API_KEY is not set. Add it to .env (see .env.example).");
    }
    return {
...
function baseURLConfig(baseURL) {
    return { configuration: { baseURL } };
}
/** GPT model — used for planning and code review. */
export function createPlannerModel(opts = {}) {
    const d = gptDefaults();
...
export function createReviewerModel(opts = {}) {
    const d = gptDefaults();
    return new ChatOpenAI({
        model: opts.model ?? d.model,
        apiKey: opts.apiKey ?? d.apiKey,
...
export function createCoderModel(opts = {}) {
