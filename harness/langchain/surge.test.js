...
import { classifySurge, computeSurgeDelayMs, invokeWithSurgeRetry, SurgeScheduler, } from "./surge.js";
// ─── Fixtures ────────────────────────────────────────────────────────────────
/** The exact incident message from the amos-saas E2E failure (2026-08-31). */
const INCIDENT = '529 {"type":"error","error":{"type":"overloaded_error","message":"The system is currently experiencing a peak-hour surge, and the server is temporarily busy. It usually recovers within 1–5 minutes. Please try again shortly (2064)"},"request_id":"06e405e2e5965672576925264941a057"}';
const SIGNAL_3MIN = {
...
describe("T1 — classifySurge", () => {
    ...
    it("parses 'retry after 30 seconds'", () => {
        const sig = classifySurge(new Error("overloaded — please retry after 30 seconds"));
        assert.ok(sig);
...
describe("T2 — computeSurgeDelayMs escalation", () => {
    it("escalates 3 → 6 → 12 min and caps at max", () => {
        const p = { jitterRatio: 0 }; // deterministic
...
describe("invokeWithSurgeRetry", () => {
    it("retries through a 529 and succeeds (the incident scenario)", async () => {
        const sleeps = [];
        let calls = 0;
        const result = await invokeWithSurgeRetry(async () => {
            calls += 1;
            if (calls < 3)
    ...
    it("rethrows non-surge errors immediately (no retry)", async () => {
        const sleeps = [];
        let calls = 0;
        await assert.rejects(invokeWithSurgeRetry(async () => {
            calls += 1;
            throw new Error("ECONNREFUSED");
        }, { policy: FAST_POLICY, sleep: async (ms) => void sleeps.push(ms) }), /ECONNREFUSED/);
...
describe("T8 — loop deps surviving a mid-graph 529", () => {
    it("writeStep throws the incident once; retry wrapper completes the loop", async () => {
        ...
        const sleeps = [];
        const state = await invokeWithSurgeRetry(() => loop.invoke({ request: "surge survival test" }, { configurable: { thread_id: "t8-surge" } }), { policy: FAST_POLICY, sleep: async (ms) => void sleeps.push(ms) });
        assert.equal(writeCalls, 2); // first surged, retry succeeded
        assert.ok(state.review, "loop produced a review");
        assert.deepEqual(sleeps, [100]);
    });
});
//# sourceMappingURL=surge.test.js.map