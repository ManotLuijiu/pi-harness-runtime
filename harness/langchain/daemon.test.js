...
function daemonTaskId(source, raw) {
    if (source === "inbox")
        return raw;
...
async function waitForEvent(topic, timeoutMs = 8000) {
    await waitFor(() => readEvents().includes(topic), timeoutMs);
}
function countEvents(topic) {
...
describe("T1 — inbox file drop triggers loop", () => {
    it("loop.started / code.written / review.completed / loop.finished appear", async () => {
        ...
        try {
            ...
            writeFileSync(join(WORKSPACE, filename), "Implement a greeting function", "utf8");
            await waitForEvent("loop.finished", 15_000);
            const events = readEvents();
            assert(events.includes("loop.started"), `Missing loop.started: ${JSON.stringify(events)}`);
            assert(events.includes("code.written"), `Missing code.written: ${JSON.stringify(events)}`);
            assert(events.includes("review.completed"), `Missing review.completed: ${JSON.stringify(events)}`);
            assert(events.includes("loop.finished"), `Missing loop.finished: ${JSON.stringify(events)}`);
        }
        finally {
            daemon.stop();
        }
...
describe("T2 — bus task.proposed event triggers loop", () => {
    it("loop.finished appears after publishing task.proposed", async () => {
        ...
        daemon.start();
        try {
            writeFileSync(join(WORKSPACE, "events.jsonl"), JSON.stringify({
                eventId: randomUUID(),
                topic: "task.proposed",
            ...
            await waitForEvent("loop.finished", 15_000);
            assert(countEvents("loop.finished") >= 1, "Expected at least one loop.finished");
        }
        finally {
            daemon.stop();
        }
...
describe("T3 — no-human proof (approvalPolicy=never)", () => {
    it("loop completes without any ack file being created", async () => {
        cleanEvents();
        const daemon = new LoopDaemon(testConfig({ sources: ["inbox"], approvalPolicy: "never" }));
        daemon.start();
        try {
            ...
            assert.strictEqual(ackFiles.length, 0, `Unexpected ack files: ${ackFiles.join(", ")}`);
        }
        finally {
            daemon.stop();
        }
...
describe("T4 — approval gate", () => {
    it("parks when approvalPolicy=always and resumes after ack", async () => {
        ...
        const daemon = new LoopDaemon(testConfig({
            sources: ["inbox"],
        ...
        try {
            ...
            assert.strictEqual(countEvents("loop.started"), 0, "loop.started should not appear before ack");
    ...
    it("task is skipped when ack is denied", async () => {
        ...
        try {
            ...
            await new Promise((r) => setTimeout(r, 500));
            assert.strictEqual(countEvents("loop.started"), 0, "No loop should start after denied ack");
        }
        finally {
            daemon.stop();
        }
...
describe("T5 — single-writer (exactly one lease winner)", () => {
    it("two daemons: only one processes the task", async () => {
        cleanEvents();
        const daemon1 = new LoopDaemon(testConfig({ agentId: "daemon-A", sources: ["inbox"] }));
        const daemon2 = new LoopDaemon(testConfig({ agentId: "daemon-B", sources: ["inbox"] }));
        daemon1.start();
        daemon2.start();
        try {
            ...
            assert.ok(finishedCount >= 1, `Expected at least 1 loop.finished, got ${finishedCount}`);
        }
        finally {
            daemon1.stop();
            daemon2.stop();
...
describe("T6 — maxIterations cap", () => {
    it("with maxIterations=2, exactly 2 code.written events appear", async () => {
        ...
        const daemon = new LoopDaemon(testConfig({ sources: ["inbox"], maxIterations: 2, deps: mockDeps }));
        daemon.start();
        try {
            ...
            assert.strictEqual(counter.n, 2, `Expected exactly 2 writes (maxIterations=2, smart-stop overridden by different files)`);
        }
        finally {
            daemon.stop();
        }
    ...
    it("smart-stop: converged (only minor comments) terminates early", async () => {
        ...
        const daemon = new LoopDaemon(testConfig({ sources: ["inbox"], maxIterations: 5, deps: mockDeps }));
        daemon.start();
        try {
...
describe("T7 — crash resilience", () => {
    it("daemon restart processes pending tasks", async () => {
        ...
        try {
            await waitForEvent("loop.finished", 15_000);
            assert.ok(countEvents("loop.finished") >= 1, "Expected loop.finished after restart");
        }
        finally {
            daemon2.stop();
        }
...
describe("T13 — persistent checkpointer (M5)", () => {
    it("loop checkpoints survive daemon stop and resume from last checkpoint", async () => {
        ...
        const daemon1 = new LoopDaemon(testConfig({ sources: ["inbox"], checkpointer: true }));
        daemon1.start();
        ...
        assert.ok(threads.length > 0, `Expected checkpoint threads, got: ${JSON.stringify(threads)}`);
        // Resume with new daemon — should pick up from checkpoint
        const daemon2 = new LoopDaemon(testConfig({ sources: ["inbox"], checkpointer: true }));
        daemon2.start();
        try {
            await waitForEvent("loop.finished", 15_000);
            assert.ok(countEvents("loop.finished") >= 1, "Expected loop.finished after resume");
        }
        finally {
            daemon2.stop();
            saver.clearAll();
