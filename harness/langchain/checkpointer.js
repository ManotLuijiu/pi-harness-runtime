...
function threadDir(root, threadId) {
    return join(root, `thread-${threadId}`);
}
function checkpointPath(threadDir, checkpointId) {
    return join(threadDir, `${checkpointId}.json`);
}
function indexPath(threadDir) {
    return join(threadDir, "index.jsonl");
}
function ensureThreadDir(root, threadId) {
    const dir = threadDir(root, threadId);
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    return dir;
}
function readIndex(threadDir) {
...
function appendIndex(threadDir, entry) {
    const path = indexPath(threadDir);
...
export class FileCheckpointSaver extends BaseCheckpointSaver {
    ...
    async _loadCheckpoint(config, dir, checkpointId) {
    ...
    async put(config, checkpoint, metadata, _newVersions) {
        ...
        const parentId = metadata.parents?.[""];
...
export function createLoopCheckpointer(workspace) {
