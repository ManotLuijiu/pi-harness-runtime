...
describe("T15 — Scoreboard blackboard wiring", () => {
    ...
    it("status.json shows phase=writing after plan step", async () => {
        ...
        const content = JSON.parse(readFileSync(join(blackboardDir, "status.json"), "utf8"));
    ...
    it("status.json shows code files after write step", async () => {
        ...
        const content = JSON.parse(readFileSync(join(blackboardDir, "status.json"), "utf8"));
        assert.ok(content.codeFiles?.some((f) => f.includes("surge.ts")), "codeFiles should include surge.ts");
    });
