...
export class LoopWidget {
    ...
    makeRenderer() {
        ...
        const factory = (_tui, theme) => {
            const fg = (color, s) => theme.fg(color, s);
            const render = (width) => {
    ...
    classifyFileTier(rec) {
        if (rec.blockers > 0)
            return "blocking";
        if (rec.writeSteps > 0 || rec.reviewsPassed > 0)
            return "warning";
        return "clean";
    }
    phaseLabel(phase) {
    ...
    fitLine(line, width) {
        ...
        for (let i = 0; i < line.length; i++) {
            ...
            if (w + charW > width - 1)
                break; // need room for the ellipsis
            w += charW;
            cut = i + (cp > 0xffff ? 2 : 1);
        }
