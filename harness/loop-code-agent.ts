...
import { createHerdrBus, ensureHerdrWorkspace } from "../packages/event-bus/src/herdr-bus.js";
...
function checkEarlyExit(record: BlackboardRecord): string | null {
	...
	const approved = Object.values(record.tasks.nodes).find(
		(n) => n.status === "done" && (n as { result?: string }).result === "approved",
	);
...
function updateTaskStatus(
	...
	if (record.tasks.nodes[taskId]) {
		(record.tasks.nodes[taskId] as unknown as { status: string; result?: string }).status = status;
		if (files) (record.tasks.nodes[taskId] as unknown as { result?: string }).result = files.join(", ");
		(record.tasks.nodes[taskId] as unknown as { updatedAt?: string }).updatedAt = new Date().toISOString();
		blackboard.save();
	}
}

function getNextReviewTask(record: BlackboardRecord, writeIteration: number): string | null {
...
function getNextWriteTask(record: BlackboardRecord, currentIteration: number): string | null {
