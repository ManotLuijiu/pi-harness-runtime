...
function findBenchPath(root: string): string | undefined {
...
async function scanDocTypeFolder(
	appPath: string,
	appName: string,
	doctypeName: string,
): Promise<{ nFields: number; isSubmittable: boolean; singleDoc: boolean }> {
...
async function estimateCustomFields(
	...
	for (const app of apps) {
		...
		if (!existsSync(fgdPath)) continue;
 ...
 */
export async function analyzeFrappe(
	workspaceRoot: string,
): Promise<FrappeAnalysis | null> {
	...
	const appsInfo: FrappeAnalysis["apps"] = [];
	const doctypesSummary: FrappeAnalysis["doctypes"] = [];
	const hooksSummaries: FrappeAnalysis["hooks"] = [];
	...
	for (const appName of appsList) {
		...
		const [hooksPath, moduleEntries] = await Promise.all([
			...
			readdir(join(appPath, appName)).catch(() => [] as string[]),
		]);
		...
		if (appDetails.isErpNext) isErpNext = true;
		if (appDetails.hasSPA) hasSPA = true;
		...
		for (const dtName of doctypeNames) {
			const { nFields, isSubmittable, singleDoc } = await scanDocTypeFolder(
				appPath,
				appName,
				dtName,
			);
