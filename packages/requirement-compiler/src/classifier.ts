...
function classifyStatement(
	...
	const goalPattern =
		/^(?:the\s+(?:system|app|user)\s+)?(allow|enable|provide|show|display|create|add|support|let|implement|build|use|send|receive|save|load|calculate|process)\b/i;
	...
	if (
		/(?:^|[\s,])given\s/i.test(stmt.originalText) ||
