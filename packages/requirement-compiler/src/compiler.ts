...
export async function compileRequirement(
	...
	const config: RequirementCompilerConfig = {
		...DEFAULT_COMPILER_CONFIG,
		...deps,
	};
	...
	if (!raw.text.trim() && !raw.title?.trim()) {
		throw new RequirementCompileError(
			...
			`Requirement ${raw.id} has no text or title.`,
	...
	const acceptanceCriteria = acceptanceNormalizer.normalize(
		classification.acceptanceCriterionStatements,
	);
	...
	if (acceptanceCriteria.length === 0 && goals.length > 0) {
		finalAcceptanceCriteria = goals.map((goal, i) => ({
			...
			outcome: [`The goal "${goal.description}" is achieved.`] as string[],
	...
	for (const stmt of allStmts) {
		...
		const pattern1 = stmt.originalText.match(
			/(?:the|an?|my)\s+([a-z][a-z0-9]*(?:\s+[a-z][a-z0-9]*)?)\s+(?:can|must|should|is|are|has|have|will|does)/gi,
		);
		if (pattern1) {
			for (const match of pattern1) {
				...
				if (
					name.length > 1 &&
					name.length < 30 &&
					!/^(?:admin|user|system|app|user account|account|any|all|some|each|the)$/i.test(
						name,
					)
				) {
		...
		const pattern2 = stmt.originalText.match(
			/(?:^|\s)([a-z][a-z0-9]{1,20})\s+can\s+(?:delete|edit|access|view|create|update|remove|manage)/gi,
		);
		if (pattern2) {
			for (const match of pattern2) {
				const name = match
					.replace(
						/\s+can\s+(?:delete|edit|access|view|create|update|remove|manage)$/i,
						"",
					)
					.trim();
				if (
					name.length > 1 &&
					name.length < 30 &&
					!/^(?:it|this|that|the|a|an|any|all|some|each|their|their own|own)$/i.test(
						name,
					)
				) {
					const key = name.toLowerCase();
	...
	for (const stmt of classification.constraints) {
		...
		if (
			/strip|will not|should not/i.test(stmt.originalText) &&
	...
	if (contradictions.length > 0) {
		throw new RequirementCompileError(
			...
			`Requirement ${raw.id} contains unresolvable contradictions.`,
			{ contradictions: contradictions.map((c) => c.question) },
		);
	}
	...
	const vaguePatterns =
		/^(maybe|perhaps|possibly|maybe perhaps|perhaps possibly|not sure|not certain|i think|we think|not sure if|uncertain|ambiguous|maybe $|possibly $|\?+$)/i;
	...
	if (
		...
		throw new RequirementCompileError(
			RequirementCompileErrorCodes.EMPTY_REQUIREMENT,
			`Requirement ${raw.id} contains no identifiable goals or constraints.`,
			{ requirementId: raw.id },
		);
	}
