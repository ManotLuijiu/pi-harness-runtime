...
function detectVagueTerms(
	statements: {
		originalText: string;
		normalizedText: string;
		sourceRef: { source: string; line?: number; text: string };
	}[],
): RequirementAmbiguity[] {
	const ambiguities: RequirementAmbiguity[] = [];
...
function detectContradictions(
	...
	function normalizeForContradiction(text: string): string {
		...
		result = result.replace(
			/\s+(?:never|no|don.t|doesn.t|can.t|cannot)\s+/gi,
			" ",
		);
		result = result.replace(/\s+not\s+/gi, " ");
		// Remove modal verbs
		result = result.replace(
			/\s+(?:must|shall|should|will|can|may|might)\s*/gi,
			" ",
		);
...
function detectScopeUncertainty(
	statements: {
		originalText: string;
		normalizedText: string;
		sourceRef: { source: string; line?: number; text: string };
...
export function detectAmbiguities(
	...
	_config: RequirementCompilerConfig,
): RequirementAmbiguity[] {
	const allStatements = [
