...
export function findModels(
	...
	if (filters.maxCostPer1M !== undefined) {
		result = result.filter(
			(m) => getCostPer1M(m.pricing) <= filters.maxCostPer1M!,
		);
	}
 ...
 */
export function getCheapestModel(models: ModelInfo[]): ModelInfo | undefined {
	if (models.length === 0) return undefined;
	return models.reduce((cheapest, current) =>
		getCostPer1M(current.pricing) < getCostPer1M(cheapest.pricing)
			? current
			: cheapest,
	);
}
 ...
 */
export function getLargestContextModel(
	models: ModelInfo[],
): ModelInfo | undefined {
	if (models.length === 0) return undefined;
 ...
 */
export function filterByCostEfficiency(models: ModelInfo[]): ModelInfo[] {
