 ...
 */
export function decomposeRequirement(input: TaskCompileInput): TaskCandidate[] {
	...
	const needsRepair = requirement.riskTags.some(
	...
	for (let i = 0; i < requirement.goals.length; i++) {
		...
		if (needsDesign) implDeps.push("design-0");
		else if (needsAnalysis) implDeps.push("analysis-0");
	...
	if (hasBrowserWorkflow && input.insertE2E !== false) reviewDeps.push("e2e-0");
...
function splitByComplexity(
	task: TaskCandidate,
	_maxComplexity: number,
): TaskCandidate[] {
	const half = Math.ceil(task.estimatedComplexity / 2) as ComplexityEstimate;
	return [
		{
			...task,
			id: `${task.id}-a`,
...
function mapGoalToCriteria(
	...
	if (goalIndex < 0) return acs.map((c) => c.id);
...
function detectBrowserWorkflow(project: ProjectProfile): boolean {
	...
	return (
		...
		testCapabilities.some((t) =>
			/playwright|cypress|selenium|e2e|browser/i.test(t.runner),
		)
	);
}
