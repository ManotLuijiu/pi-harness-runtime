 ...
 */
export function assertNoCycles(graph) {
    ...
    if (visited.length !== graph.tasks.length) {
        const unvisited = graph.tasks
            ...
            .map((t) => t.id);
        throw new TaskCompilerError(TaskCompilerErrorCode.CYCLIC_DEPENDENCY, `Cyclic dependency detected. Tasks in cycle: ${unvisited.join(", ")}`, { cyclicTasks: unvisited });
    }
}
/**
 ...
 */
export function assertNoExclusiveFileOverlap(graph) {
    const exclusiveTasks = graph.tasks.filter((t) => t.fileOwnership.mode === "exclusive");
    const overlaps = [];
    ...
    if (overlaps.length > 0) {
        const details = overlaps.map((o) => `${o.taskA} ↔ ${o.taskB}: ${o.files.join(", ")}`);
        throw new TaskCompilerError(TaskCompilerErrorCode.FILE_OVERLAP_CONFLICT, `Exclusive file overlap detected between parallel tasks: ${details.join("; ")}`, { overlaps });
    }
}
/**
 * Assert every task has at least one required verification output.
 */
export function assertEveryTaskHasVerification(graph) {
    const missing = [];
    for (const task of graph.tasks) {
    ...
    if (missing.length > 0) {
        throw new TaskCompilerError(TaskCompilerErrorCode.NO_VERIFICATION, `Tasks without required verification outputs: ${missing.join(", ")}`, { taskIds: missing });
    }
}
/**
 * Assert no task has an empty objective.
 */
export function assertNoEmptyObjectives(graph) {
    const empty = [];
    for (const task of graph.tasks) {
    ...
    if (empty.length > 0) {
        throw new TaskCompilerError(TaskCompilerErrorCode.EMPTY_OBJECTIVE, `Tasks with empty objectives: ${empty.join(", ")}`, { taskIds: empty });
    }
}
/**
 * Assert every acceptance criterion is assigned to at least one task.
 */
export function assertEveryCriterionAssigned(graph, criterionIds) {
...
function patternSubsumes(parent, child) {
