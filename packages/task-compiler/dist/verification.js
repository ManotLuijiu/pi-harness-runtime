 ...
 */
export function assignVerification(tasks, requirement) {
...
function buildImplementationOutputs(task, requirement) {
    ...
    const goal = requirement.goals.find((g) => task.objective.includes(g.description.slice(0, 30)));
    if (goal) {
        outputs.push({
            kind: "verification",
    ...
    const hasDestructive = requirement.riskTags.some((r) => r.risk === "destructive_operation");
    if (hasDestructive) {
        outputs.push({
            kind: "verification",
...
function buildTestOutputs(task) {
    const outputs = [
        {
            kind: "test_result",
...
function buildReviewOutputs(task) {
    const outputs = [
        {
            kind: "report",
...
function buildAnalysisOutputs(task) {
    return [
        {
            kind: "report",
...
function buildDesignOutputs(task) {
    return [
        {
            kind: "schema",
...
function buildRepairOutputs(task) {
    return [
        {
            kind: "verification",
...
function buildDocumentationOutputs(task) {
    return [
        {
            kind: "file",
            path: "README.md",
