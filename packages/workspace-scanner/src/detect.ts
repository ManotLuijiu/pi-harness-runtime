...
export function detectProject(rootPath: string): ProjectConfig {
	...
	if (existsSync(packageJsonPath)) {
		try {
			...
			if (pkg.name) config.name = String(pkg.name);
			config.language = "TypeScript";
			if (pkg.workspaces) config.packageManager = "npm";
			if (existsSync(join(rootPath, "pnpm-lock.yaml"))) config.packageManager = "pnpm";
			if (existsSync(join(rootPath, "yarn.lock"))) config.packageManager = "yarn";
			if (existsSync(join(rootPath, "bun.lockb"))) config.packageManager = "bun";
		} catch { /* ignore */ }
	}
