...
export function detectAssertions(source) {
    const assertions = [];
...
function extractFunctionName(content) {
    const match = content.match(/(?:function\s+(\w+)|export\s+const\s+(\w+)|(\w+)\s*=\s*(?:async\s*)?\()/);
    return match?.[1] ?? match?.[2] ?? match?.[3];
}
/** Get framework-specific imports. */
function getImports(framework) {
...
function generateTestBlocks(fnName, framework) {
...
export function generateUnitTests(source, framework = "bun") {
...
export function generateE2ETests(scenarios) {
...
export function generateMockFixtures(source) {
    const typeMap = {
        string: '"mock-string"',
        number: "42",
        boolean: "true",
