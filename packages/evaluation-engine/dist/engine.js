...
function findMissingOutputs(requiredArtifacts, outputs) {
    ...
    for (const required of requiredArtifacts) {
        ...
        const found = outputPaths.has(required) ||
...
function analyzeAcceptanceCoverage(acceptanceCriteria, outputs, testResults) {
    ...
    for (const criterion of acceptanceCriteria) {
        ...
        const outputKeywordMatches = keywords.filter((word) => outputText.includes(word)).length;
        const coveredByOutput = keywords.length > 0 &&
            outputKeywordMatches >= Math.ceil(keywords.length / 2);
        const coveredByPassingTests = passingTests.length > 0 &&
...
function calculateScore(weights, testAnalysis, acceptanceCoverage, criticalPolicies, securityFindings, regressionFindings) {
    ...
    const regressionPenalty = regressionFindings.length * weights.regressionPenalty;
    breakdown.regressionPenalty = -regressionPenalty;
    const totalScore = Math.max(0, Math.min(100, correctnessScore +
        testScore +
        acceptanceScore +
        qualityScore -
        policyPenalty -
...
function classifyFailure(findings) {
    ...
    for (const finding of findings) {
        if (finding.dimension === "test_completeness" &&
            finding.severity === "critical") {
            if (!classes.includes("test_failure"))
                classes.push("test_failure");
        }
        if (finding.dimension === "correctness" && finding.severity === "high") {
            if (!classes.includes("missing_output"))
                classes.push("missing_output");
        }
        if (finding.dimension === "acceptance_coverage" &&
            finding.severity === "medium") {
            if (!classes.includes("acceptance_gap"))
                classes.push("acceptance_gap");
        }
        ...
        if (finding.dimension === "security_risk") {
            if (!classes.includes("security_issue"))
                classes.push("security_issue");
        }
        if (finding.dimension === "regression_risk") {
            if (!classes.includes("regression_risk"))
                classes.push("regression_risk");
        }
    }
...
export class EvaluationEngine {
    ...
    evaluate(request) {
        ...
        const acceptanceAnalysis = analyzeAcceptanceCoverage(task.acceptanceCriteria, outputs, testResults);
        allFindings.push(...acceptanceAnalysis.findings);
        // 5. Check for security issues
        const securityFindings = allFindings.filter((f) => f.dimension === "security_risk");
        // 6. Check for regression risks
        const regressionFindings = allFindings.filter((f) => f.dimension === "regression_risk");
        // 7. Calculate score
        const { score, breakdown: _breakdown } = calculateScore(this.weights, testAnalysis, acceptanceAnalysis.coverage, policyAnalysis.criticalFindings, securityFindings, regressionFindings);
        // 8. Collect evidence
        ...
        else if (acceptanceAnalysis.coverage < 80 &&
