...
export function compactToBudget(
	...
	const optionalSectionTokens: Map<number, number> = new Map();
	...
	for (const { section, index } of compactableSections) {
		...
		const avgTokensPerLine =
			lines.length > 0 ? Math.ceil(sectionTokens / lines.length) : 1;
		...
		while (trimmedLines.length > 0 && overBudgetBy > 0) {
			...
			const isLowPriority = priorityKeywords.some((kw) =>
				lastLine.toUpperCase().includes(kw),
			);
