...
export function createSyncHasher() {
	let sha256Impl: ((content: string) => string) | null = null;
	return {
		sha256(content: string): string {
			if (!sha256Impl) {
...
export async function compilePrompt(
	...
	const availableTokens =
		profile.maximumInputTokens - profile.reservedOutputTokens;

	const estimateSectionTokens = (content: string) => estimateTokens(content);

	const budgeted = compactToBudget(
		sections,
		availableTokens,
