 ...
 */
export function validateSections(sections, request, _options) {
    const errors = [];
    ...
    const requiredKinds = new Set(sections.filter((s) => s.required).map((s) => s.kind));
    ...
    for (const section of sections) {
        if (section.required) {
            for (const ref of section.sourceRefs) {
                const refText = typeof ref.text === "string" ? ref.text : String(ref.text ?? "");
                if (!refText.trim()) {
                    errors.push({
 ...
 */
export function validateTokenBudget(estimatedTokens, profile) {
    const available = profile.maximumInputTokens - profile.reservedOutputTokens;
    if (estimatedTokens > available) {
        throw new PromptCompileError("TOKEN_BUDGET_EXCEEDED", `Estimated ${estimatedTokens} tokens exceeds available budget ${available}`, {
            estimatedTokens,
...
function isProhibitedTool(permission) {
