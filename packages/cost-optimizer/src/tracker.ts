 ...
 */
export function getPeriodBoundaries(period: CostPeriod): {
	start: Date;
	end: Date;
} {
	const now = new Date();
 ...
 */
export function isInPeriod(timestamp: string, period: CostPeriod): boolean {
 ...
 */
export function getTotalCost(entries: CostEntry[], period: CostPeriod): number {
