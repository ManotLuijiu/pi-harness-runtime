 ...
 */
export function calculateQualityImpact(current, candidate) {
    return current.qualityScore - candidate.qualityScore;
}
/**
 * Get quality score for a model
 */
export function getQualityScore(model, requiredCapabilities) {
    // Base score
    let score = model.qualityScore;
    // Bonus for matching required capabilities
    const matchedCaps = requiredCapabilities.filter((cap) => model.capabilities.includes(cap)).length;
    score += matchedCaps * 5;
 ...
 */
export function findCheaperAlternatives(currentModel, requiredCapabilities, options) {
 ...
 */
export function generateSwitchRecommendation(currentModel, candidate, inputTokens, outputTokens) {
    const currentCost = calculateModelCost(currentModel, inputTokens, outputTokens);
    const candidateCost = calculateModelCost(candidate, inputTokens, outputTokens);
 ...
 */
export function optimizeModelSelection(requirements, models = DEFAULT_MODELS) {
    // Filter by requirements
    const candidates = models.filter((m) => {
        if (m.contextWindow < requirements.minContextWindow)
            return false;
        ...
        if (requirements.requiredCapabilities?.length) {
            const hasAllCaps = requirements.requiredCapabilities.every((cap) => m.capabilities.includes(cap));
            if (!hasAllCaps)
                return false;
        }
        return true;
    ...
    const scored = candidates.map((m) => {
        const estimatedCost = calculateModelCost(m, requirements.estimatedInputTokens, requirements.estimatedOutputTokens);
        const qualityScore = getQualityScore(m, requirements.requiredCapabilities);
        // Tradeoffs
        const tradeoffs = [];
        if (m.pricing.inputPer1M > 1)
            tradeoffs.push("Higher input cost");
        if (m.qualityScore < 85)
            tradeoffs.push("Lower quality rating");
        return {
