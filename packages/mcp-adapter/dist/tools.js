...
export const HARNESS_TOOLS = [
    {
        ...
        description: "Analyze a codebase directory to detect framework, structure, and dependencies",
        inputSchema: {
            type: "object",
            properties: {
                path: {
    ...
    {
        name: "search_memory",
        description: "Search the harness persistent memory store for relevant knowledge",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "list_skills",
        description: "List all registered harness skills with their triggers and capabilities",
        inputSchema: {
            type: "object",
            properties: {
                capability: {
    ...
    try {
        switch (toolName) {
            case "analyze_workspace":
                return handleAnalyzeWorkspace(args);
            case "search_memory":
                return handleSearchMemory(args);
            case "list_skills":
                return handleListSkills(args);
            case "invoke_skill":
                return handleInvokeSkill(args);
            default:
                return {
...
async function handleAnalyzeWorkspace(args) {
    ...
    if (files.includes("composer.json"))
        frameworks.push("php");
    if (files.includes("frappe-bench") ||
    ...
    const result = JSON.stringify({ path, frameworks, fileCount: files.length }, null, 2);
