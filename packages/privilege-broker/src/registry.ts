...
export const DEFAULT_PRIVILEGES_PATH = join(
 ...
 */
export function loadRegistry(path?: string): PrivilegeRegistry {
	const filePath = path ?? DEFAULT_PRIVILEGES_PATH;
	if (!existsSync(filePath)) {
		throw new PrivilegeRegistryError(
			`privileges file not found at ${filePath}. Copy config/privileges.yaml.example to config/privileges.yaml and edit it.`,
		);
	}
	...
	const registry = raw as PrivilegeRegistry;
