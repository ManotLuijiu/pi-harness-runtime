...
async function detectVersion(root: string): Promise<string | undefined> {
	...
	if (!existsSync(composerPath)) return undefined;
...
async function detectAuth(root: string): Promise<string | undefined> {
	...
	if (packages.includes("laravel/sanctum")) return "sanctum";
	if (packages.includes("laravel/passport")) return "passport";
	if (packages.includes("laravel/spark-aurelius")) return "spark";
	if (packages.includes("tymon/jwt-auth")) return "jwt";
		...
		try {
			...
			for (const entry of entries) {
				if (entry.name.startsWith(".") || entry.name === "Controller.php")
					continue;
				if (
					entry.name.endsWith("Controller.php") ||
					...
					for (const sub of subEntries) {
						if (
							sub.endsWith("Controller.php") ||
							sub.endsWith("Controller.ts")
						) {
							controllers.push(
								`${entry.name}/${sub.replace(/\.(php|ts)$/, "")}`,
							);
						}
					}
				}
...
async function findModels(root: string): Promise<string[]> {
	...
	for (const dir of modelDirs) {
		...
		try {
			...
			for (const entry of entries) {
				if (entry.startsWith(".") || entry === "User.php") continue;
...
async function findCommands(root: string): Promise<string[]> {
	...
	for (const dir of commandDirs) {
		...
		try {
			const entries = await readdir(dir);
			for (const entry of entries) {
				if (entry.startsWith(".") || !entry.endsWith(".php")) continue;
...
export async function analyzeLaravel(
	root: string,
): Promise<LaravelAnalysis | null> {
	if (!isLaravelWorkspace(root)) return null;
	...
	const [controllers, models, views, commands, migrations, envVars] =
		await Promise.all([
