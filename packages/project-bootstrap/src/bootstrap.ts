...
export function validateSpec(spec: ProjectSpec): ValidationError[] {
  ...
  if (!spec.type || !['monorepo', 'single-package', 'multi-package'].includes(spec.type)) {
    errors.push({ field: 'type', message: 'Type must be monorepo, single-package, or multi-package' });
  }

  if (!spec.frameworks || spec.frameworks.length === 0) {
    errors.push({ field: 'frameworks', message: 'At least one framework is required' });
  }
 ...
 */
export function getTemplateForSpec(spec: ProjectSpec, override?: string): Template {
  ...
  if (spec.frameworks.includes('node') || spec.frameworks.includes('typescript')) {
