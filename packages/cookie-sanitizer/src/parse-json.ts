...
interface EditThisCookieRecord {
	domain?: unknown;
 ...
 */
export function parseEditThisCookieJson(text: string): CanonicalCookie[] {
	...
	const records: EditThisCookieRecord[] = [];
	if (Array.isArray(data)) {
		for (const item of data) {
			if (item && typeof item === "object" && !Array.isArray(item)) {
				records.push(item as EditThisCookieRecord);
			}
		}
	} else if (data && typeof data === "object" && !Array.isArray(data)) {
		records.push(data as EditThisCookieRecord);
	}
...
function asExpires(v: unknown, session: unknown): number | undefined {
	if (session === true || session === "true") return undefined;
