 ...
 */
export function enumerateDropFiles(
	dir: string,
	excludePath?: string,
): string[] {
	...
	function walk(current: string, depth: number) {
		...
		for (const name of entries) {
			...
			if (name.endsWith("~") || name.endsWith(".swp")) continue;
...
export function readInputFile(filePath: string): {
	...
	if (!stat.isFile()) return { text: "", reason: "not a regular file" };
...
export function sync(opts: SyncOptions = {}): SyncResult {
	...
	const allCookies: CanonicalCookie[] = [];
	let firstBasename: string | undefined;
	...
	for (const filePath of files) {
		...
		if (!firstBasename) firstBasename = det.basename;
	}
 ...
 */
export function hasAnyCookieSource(
	dropDir: string = DEFAULT_DROP_DIR,
): boolean {
 ...
 */
export function inferCachePath(
	dropDir: string = DEFAULT_DROP_DIR,
): string | null {
	...
	for (const fp of files) {
		...
		if (!det.format) continue;
		if (det.format === "netscape") cookies.push(...parseNetscape(text));
		else if (det.format === "edit-this-cookie-json")
