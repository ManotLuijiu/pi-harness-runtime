...
export function domainMatchesProvider(
	domain: string,
	provider: ProviderId,
): boolean {
...
export function isExpired(
	...
	if (cookie.expires == null) return false; // session cookie
	if (cookie.expires <= 0) return false; // 0 = session in Netscape
...
export function normalize(
	...
	const dedup = new Map<string, CanonicalCookie>();
	...
	for (const c of cookies) {
		...
		if (provider && !domainMatchesProvider(c.domain, provider)) continue;
