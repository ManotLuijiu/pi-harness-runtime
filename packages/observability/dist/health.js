...
export class HealthMonitor {
     ...
     */
    registerHealthCheck(name, check, critical = true) {
    ...
    async checkHealthAll() {
        ...
        const results = await Promise.allSettled(Array.from(this.checks.entries()).map(async ([_name, registration]) => {
            try {
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Health check timeout")), 5000));
    ...
    async checkReadiness() {
        ...
        if (report.overall === "degraded") {
            return {
                ...
                details: {
                    failedChecks: Object.keys(report.checks).filter((name) => report.checks[name].status !== "healthy"),
                },
                timestamp: new Date().toISOString(),
            };
        }
 ...
 */
export function createConnectivityHealthCheck(_name, endpoint, timeout = 5000) {
    return async () => {
        try {
 ...
 */
export function createMemoryHealthCheck(_name, thresholdPercent = 90) {
    return async () => {
 ...
 */
export function createDiskSpaceHealthCheck(_name, _minFreeMB = 100) {
    return async () => {
 ...
 */
export function createHealthMonitor(serviceName, version) {
