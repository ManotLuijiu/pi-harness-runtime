...
import type { ProjectType, ProjectDetection } from "./types.js";

const SIGNALS: Record<ProjectType, Array<{ pattern: RegExp; weight: number }>> = {
  frappe_erpnext: [
...
const SEED_STRATEGIES: Record<ProjectType, string> = {
...
const E2E_STRATEGIES: Record<ProjectType, string> = {
...
export function detectProjectType(filePaths: string[]): ProjectDetection {
  ...
  for (const filePath of filePaths) {
    for (const [pType, pSignals] of Object.entries(SIGNALS) as [ProjectType, typeof SIGNALS.frappe_erpnext][]) {
  ...
  const projectType = best[1] > 0 ? (best[0] as ProjectType) : "unknown";
...
export function parseSignals(entries: string[]): string[] {
