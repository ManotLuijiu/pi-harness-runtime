...
import type { DetectedSymbol, GeneratedDocs, DocSection, SourceFile } from "./types.js";
...
export function detectSymbols(source: SourceFile): DetectedSymbol[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const fnMatch = trimmed.match(/^(?:async\s+)?(?:export\s+)?function\s+(\w+)\s*\(([^)]*)\)/);
    if (fnMatch) {
      symbols.push({
        name: fnMatch[1],
        kind: "function",
...
export function generateDocs(sources: SourceFile[], projectSignals: string[]): GeneratedDocs {
