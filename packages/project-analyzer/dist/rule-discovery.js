 ...
 */
export async function parseRuleFile(file) {
 ...
 */
export async function discoverRuleFiles(rootPath, fs, fileNames = [
    "AGENTS.md",
    "RULES.md",
    "PROJECT_RULES.md",
    "CONTRIBUTING.md",
 ...
 */
export function mergeRules(ruleFiles, prioritizeUserDefined = true) {
 ...
 */
export function extractCommandsFromRules(rules) {
    const permitted = [];
    const prohibited = [];
    const commandPattern = /`([^`]+)`/g;
    const permitPattern = /permi(ted|ssion)|allow|can\s+run|may\s+execute/i;
    const prohibitPattern = /prohibit|forbid|deny|block|cannot\s+run|may\s+not\s+execute/i;
 ...
 */
export function extractMetadataFromRules(rules) {
    const metadata = {};
    for (const rule of rules) {
