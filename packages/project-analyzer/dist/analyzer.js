...
import { AnalysisCache, hashRuleFiles, hashManifestFiles, hashFrameworkConfigFiles, } from "./cache.js";
 ...
 */
export async function detectTestCapabilities(fs, framework) {
    const capabilities = [];
...
export class ProjectAnalyzer {
    ...
    async detectFrameworks(fs) {
        ...
        pluginResults.sort((a, b) => b.framework.confidence - a.framework.confidence);
     ...
     */
    async discoverRules(rootPath, fs) {
     ...
     */
    async discoverCommands(rootPath, _fs) {
        const allCommands = [];
        // Parse package.json if exists
        const packageJson = await this.parseJsonFile(join(rootPath, "package.json"));
        if (packageJson) {
            allCommands.push(...parsePackageJsonScripts(packageJson));
        }
        // Parse composer.json if exists
        const composerJson = await this.parseJsonFile(join(rootPath, "composer.json"));
        if (composerJson) {
            allCommands.push(...parseComposerScripts(composerJson));
        }
        // Parse pyproject.toml if exists
        const pyprojectToml = await this.parseJsonFile(join(rootPath, "pyproject.toml"));
        if (pyprojectToml) {
    ...
    async analyze(request) {
        ...
        try {
            ...
            // Get git revision
            const revision = request.revision || (await this.getGitRevision(rootPath));
            ...
            // Try cache first
            const ruleFiles = await discoverRuleFiles(rootPath, fs);
            const ruleHash = await hashRuleFiles(ruleFiles.map((f) => ({ path: f.path, content: f.content })));
            const manifestFiles = scanResult.files
                .filter((f) => ["package.json", "pyproject.toml", "composer.json"].includes(basename(f.path)))
                .map((f) => ({
                path: f.path,
                content: walker.readFileSafe(f.path),
            }));
            const manifestContents = await Promise.all(manifestFiles.map((f) => f.content));
            const manifestHash = await hashManifestFiles(manifestFiles.map((f, i) => ({
                path: f.path,
                content: manifestContents[i] || "",
            })));
            const frameworkConfigFiles = scanResult.files
                .filter((f) => f.relativePath.includes("config") ||
                [
                    "next.config.js",
                    "vite.config.ts",
                    "tsconfig.json",
            ...
            const frameworkConfigContents = await Promise.all(frameworkConfigFiles.map((f) => f.content));
            const configHash = await hashFrameworkConfigFiles(frameworkConfigFiles.map((f, i) => ({
                path: f.path,
            ...
            // Detect frameworks
            const { frameworks, warnings: frameworkWarnings } = await this.detectFrameworks(fs);
            ...
            const testCapabilities = await detectTestCapabilities(fs, primaryFramework?.category || "unknown");
            ...
            if (request.config?.detectMonorepo !== false) {
                ...
                for (const subdir of subdirs) {
                    const packageJson = await this.parseJsonFile(join(rootPath, subdir, "package.json"));
            ...
            const avgFrameworkConfidence = frameworks.length > 0
                ...
                : 0;
            // Build warnings
            const warnings = [...frameworkWarnings];
                    ...
                    ? [
                        {
                            ...
                            configPath: packageManagerFiles.find((f) => f.includes(primaryPackageManager)) || "",
                            primary: true,
                        },
                    ]
                    : [],
