 ...
 */
export const GENERIC_SIGNALS: SignalDefinition[] = [
	// Frappe signals
...
export async function scanSignals(
	...
	signals: SignalDefinition[] = GENERIC_SIGNALS,
 ...
 */
export function groupSignalsByFramework(
	signals: DetectionSignal[],
	frameworkMap: Map<FrameworkCategory, SignalDefinition[]>,
): Map<FrameworkCategory, DetectionSignal[]> {
	const grouped = new Map<FrameworkCategory, DetectionSignal[]>();
...
export class GenericFrameworkDetector {
	...
	private signalsPerFramework: Map<FrameworkCategory, SignalDefinition[]>;

	constructor(signals: SignalDefinition[] = GENERIC_SIGNALS) {
	 ...
	 */
	async detect(fs: ReadonlyFileSystem): Promise<DetectionResult[]> {
		const allSignals = await scanSignals(fs, this.signals);
		const grouped = groupSignalsByFramework(
			allSignals,
			this.signalsPerFramework,
		);
...
export function createCompositePlugin(
	customPlugins: FrameworkAnalyzerPlugin[],
): FrameworkAnalyzerPlugin {
