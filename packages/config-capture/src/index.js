 ...
 */
export function detectConfig(result) {
...
function detectArrayOfObjects(result) {
...
function extractSampleData(result, _type) {
 ...
 */
export function removeSensitiveFields(config) {
    const sanitized = {};
 ...
 */
export function formatForAgentsMd(detection, sampleData) {
    const lines = [];
 ...
 */
export function registerConfigCapture(pi, config = {}) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    // Track if we've already suggested documenting this config in this session
    const documentedConfigs = new Set();
    ...
    pi.on("tool_execution_end", (event) => {
        ...
        if (!apiTools.includes(event.toolName))
            return;
        ...
        if (documentedConfigs.has(configKey))
            return;
        // Log detection
        if (cfg.debug) {
