...
// Built-in evaluators
export const exactMatchEvaluator = async (input, output) => {
...
export const substringMatchEvaluator = async (input, output) => {
    const expected = input.expectedOutput ?? "";
...
export const lengthEvaluator = async (input, output) => {
    const min = input.metadata?.["minLength"] ?? 0;
    const max = input.metadata?.["maxLength"] ?? Infinity;
