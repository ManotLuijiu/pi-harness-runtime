...
async function detectFramework(root: string): Promise<WebFrameworkType> {
	...
	if (deps["rails"] || deps["@rails/ujs"]) return "rails";
	if (deps["laravel-mix"] || deps["laravel"]) return "laravel";
	if (scripts["nuxt"] || scripts["nuxt:build"]) return "nuxt";
	if (scripts["astro"] || scripts["astro:build"]) return "astro";
...
function guessComponentName(framework: WebFrameworkType, path: string): string {
...
function getApiPatterns(framework: WebFrameworkType): ApiPattern[] {
	switch (framework) {
		case "express":
		case "fastify":
		case "koa":
...
function extractHttpMethods(filePath: string): string[] {
	try {
		...
		if (/\bget\b/i.test(content)) methods.add("GET");
		if (/\bpost\b/i.test(content)) methods.add("POST");
		if (/\bput\b/i.test(content)) methods.add("PUT");
		if (/\bpatch\b/i.test(content)) methods.add("PATCH");
		if (/\bdelete\b/i.test(content)) methods.add("DELETE");
		if (/\boptions\b/i.test(content)) methods.add("OPTIONS");
