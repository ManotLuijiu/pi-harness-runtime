...
function extractRepairPatterns(repairs) {
    const patterns = [];
    ...
    for (const [repairType, typeRepairs] of byType) {
        ...
        patterns.push({
            ...
            successIndicators: successRate > 0.5 ? [`effective for ${successCount} tasks`] : [],
            failureIndicators: successRate <= 0.5
...
function extractFailurePatterns(events) {
    const patterns = [];
...
function createExperience(pattern, scope, humanFeedback) {
    ...
    const factors = {
        ...
        consistentPositiveOutcomes: pattern.successIndicators.length * pattern.occurrenceCount,
...
export class LearningEngine {
    ...
    learn(request) {
        ...
        const providerPatterns = extractProviderSuccessPatterns(request.providerMetrics);
        for (const pattern of providerPatterns) {
            const experience = createExperience(pattern, "task_type", request.humanFeedback);
        ...
        for (const pattern of failurePatterns) {
            const experience = createExperience(pattern, "global", request.humanFeedback);
            experiences.push(experience);
        }
        ...
        if (request.humanFeedback) {
            for (const feedback of request.humanFeedback) {
                if (feedback.approved) {
                    ...
                    const existing = experiences.find((e) => e.pattern.toLowerCase().includes(feedback.taskId.toLowerCase()));
                    if (existing) {
