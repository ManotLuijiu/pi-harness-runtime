...
export function isBreakingChange(message) {
    const lower = message.toLowerCase();
    return (lower.includes("breaking change") ||
        lower.includes("breaking:") ||
        lower.includes("!:"));
}
export function extractScope(message) {
...
export function buildChanges(commits) {
    const changes = {
        added: [],
        changed: [],
        deprecated: [],
...
export function createChangelogEntry(version, commits, options) {
    const hasBreaking = commits.some((c) => c.type === "break" || isBreakingChange(c.message));
    const hasSecurity = commits.some((c) => c.message.toLowerCase().includes("security"));
    return {
...
export function formatChangelogMarkdown(entry) {
    const lines = [];
    lines.push(`## ${entry.version}${entry.breaking ? " **BREAKING**" : ""} (${entry.date})`);
    lines.push("");
