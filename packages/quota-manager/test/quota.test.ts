...
describe("parseMiniMaxError", () => {
  it("parses quota error code 2056", () => {
    const result = parseMiniMaxError({ message: "Error code: 2056 - Rate limit exceeded" });
  ...
  it("extracts retry time in seconds", () => {
    const result = parseMiniMaxError({ message: "rate limit exceeded. Retry after 30 seconds" });
    expect(result).not.toBeNull();
    expect(result!.retryAfterMs).toBe(30000);
  });

  it("extracts retry time in minutes", () => {
    const result = parseMiniMaxError({ message: "rate limit exceeded. Retry after 2 minutes" });
    expect(result).not.toBeNull();
...
describe("parseOpenAIError", () => {
  it("parses insufficient_quota", () => {
    const result = parseOpenAIError({ code: "insufficient_quota", message: "Quota exceeded" });
    expect(result).not.toBeNull();
  ...
  it("parses context_length_exceeded", () => {
    const result = parseOpenAIError({ code: "context_length_exceeded", message: "Too many tokens" });
    expect(result).not.toBeNull();
    expect(result!.exhausted).toBe(true);
  });

  it("parses 429 rate limit", () => {
    const result = parseOpenAIError({ message: "HTTP 429: Rate limit exceeded" });
    expect(result).not.toBeNull();
    expect(result!.exhausted).toBe(true);
  ...
  it("returns null for unrelated errors", () => {
    const result = parseOpenAIError({ code: "invalid_request_error", message: "Bad request" });
    expect(result).toBeNull();
  });
});
...
describe("TUIUsageMonitor", () => {
  ...
  describe("detectProvider", () => {
    it("detects OpenAI", () => {
      expect(monitor.detectProvider("OpenAI GPT-4 quota exceeded")).toBe("openai");
    });
    ...
    it("detects Anthropic Claude", () => {
      expect(monitor.detectProvider("Claude overloaded_error")).toBe("anthropic");
    });

    it("detects OpenRouter", () => {
      expect(monitor.detectProvider("OpenRouter quota exhausted")).toBe("openrouter");
    });
  ...
  describe("processMessage", () => {
    it("processes OpenAI quota message", () => {
      const signal = monitor.processMessage("OpenAI insufficient_quota: daily limit reached");
      expect(signal).not.toBeNull();
      expect(signal!.provider).toBe("openai");
    });

    it("processes rate limit message", () => {
      const signal = monitor.processMessage("Claude rate_limit_error: try again in 30 seconds");
      expect(signal).not.toBeNull();
    ...
    it("processes context window message", () => {
      const signal = monitor.processMessage("OpenAI context_length_exceeded: too many tokens");
      expect(signal).not.toBeNull();
