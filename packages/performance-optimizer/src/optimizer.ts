 ...
 */
function detectBottlenecks(metrics: RuntimeMetricSnapshot): string[] {
 ...
 */
function normalizeProviderStats(
	providerMetrics: Record<
		string,
		{ callCount: number; successCount: number; failureCount: number }
	>,
): Record<string, { successRate: number; failureRate: number }> {
 ...
 */
function suggestContextReduction(
	...
	const reductionPercent =
		((currentContextTokens - targetTokens) / currentContextTokens) * 100;
 ...
 */
function suggestProviderChange(
	metrics: RuntimeMetricSnapshot,
	...
	const availableProviders = providerStates.filter(
		(p) => p.state === "available" && p.providerId !== worstProvider,
	);
 ...
 */
function suggestParallelismChanges(
	metrics: RuntimeMetricSnapshot,
	providerStates: ProviderRuntimeState[],
	policy: PerformancePolicy,
	taskGraph?: { nodes: Array<{ id: string; status: string }> },
): PerformanceRecommendation[] {
	...
	if (taskGraph) {
		...
		const limitedProviders = providerStates.filter(
			(p) => p.state === "limited",
		);
...
function resolveConflicts(
	...
	const resolved: PerformanceRecommendation[] = [];
...
function calculateImpact(
	metrics: RuntimeMetricSnapshot,
	recommendations: PerformanceRecommendation[],
): PerformanceOptimizationPlan["expectedImpact"] {
	...
	return {
		tokenReductionPercent:
			tokenReduction > 0 ? Math.min(tokenReduction, 50) : undefined,
		latencyReductionPercent:
			latencyReduction > 0 ? Math.min(latencyReduction, 40) : undefined,
		retryReductionPercent:
			retryReduction > 0 ? Math.min(retryReduction, 30) : undefined,
	};
}

export class PerformanceOptimizer {
	private policy: PerformancePolicy;
	...
	analyze(
		...
		try {
			...
			// Provider change
			const providerRec = suggestProviderChange(
				metrics,
				providerStates,
				this.policy,
			);
			...
			// Parallelism changes
			const parallelismRecs = suggestParallelismChanges(
				metrics,
				providerStates,
				this.policy,
				request.taskGraph,
	...
	private eventListeners: Array<(event: PerformanceOptimizerEvent) => void> =
		[];
	...
	private emit(event: PerformanceOptimizerEvent): void {
