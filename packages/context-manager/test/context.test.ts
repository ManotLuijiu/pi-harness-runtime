...
const makeItem = (overrides?: Partial<import("../src/index.js").ContextItem>): import("../src/index.js").ContextItem => ({
...
describe("getPressureLevel", () => {
  it("normal below warning", () => expect(getPressureLevel(0.3)).toBe("normal"));
  it("warning above warningThreshold", () => expect(getPressureLevel(0.65)).toBe("warning"));
  it("compact above compactThreshold", () => expect(getPressureLevel(0.75)).toBe("compact"));
  it("hard_stop above hardStopThreshold", () => expect(getPressureLevel(0.9)).toBe("hard_stop"));
  it("uses custom policy", () => {
    const custom = { warningThreshold: 0.1, compactThreshold: 0.2, hardStopThreshold: 0.3 };
...
describe("compactContext", () => {
  it("extracts decisions", () => {
    const items = [
      makeItem({ content: "DECISION: use tRPC for API" }),
    ];
  ...
  it("extracts questions", () => {
    const items = [
      makeItem({ content: "Should we use Redis? TODO: decide caching strategy" }),
    ];
    const result = compactContext(items, 1000);
...
describe("formatResumeMarkdown", () => {
  it("includes headings", () => {
    const prompt = generateResumePrompt(
      { decisions: ["Use tRPC"], openQuestions: ["Which DB?"], taskProgress: {}, remainingTokens: 100, compactedContent: "" },
      "Continue",
    );
