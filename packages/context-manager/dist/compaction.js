...
function extractTaskProgress(messages) {
    const progress = {};
    for (const msg of messages) {
        for (const line of msg.content.split("\n")) {
            const match = line.match(/^(?:progress|task|step)[:\s]+([^|]+)\|\s*(.+)/i);
            if (match)
                progress[match[1].trim()] = match[2].trim();
        }
    }
    return progress;
}
/** Compact context by extracting durable state and keeping high-priority items. */
export function compactContext(messages, maxTokens, policy = DEFAULT_POLICY) {
...
export function generateResumePrompt(result, nextAction) {
    const summary = [
        "## Session Summary",
...
export function formatResumeMarkdown(prompt) {
    const lines = [
        "# Resume Prompt",
        "",
        prompt.summary,
