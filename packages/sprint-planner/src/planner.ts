...
export function createSprintConfig(
...
export function sortRequirements(
	reqs: Requirement[],
	strategy: SprintConfig["prioritizationStrategy"],
): Requirement[] {
...
function topologicalSort(reqs: Requirement[]): Requirement[] {
	...
	while (remaining.size > 0) {
		...
		if (!added) {
			for (const id of remaining) result.push(byId.get(id)!);
			break;
		}
	}
...
export function planSprints(
	...
	config?: Partial<SprintConfig>,
): PlanningResult {
	const cfg = createSprintConfig(config);
	const allTasks: Task[] = requirements.flatMap((req) =>
		decomposeRequirement(req, { defaultEstimate: cfg.defaultEstimate }),
	);
...
function makeSprint(number: number, tasks: Task[]): Sprint {
...
export function calculateVelocity(sprints: Sprint[]): number {
	const completed = sprints.filter((s) =>
		s.tasks.every((t) => t.status === "done"),
	);
	if (completed.length === 0) return 0;
	return (
		completed.reduce((s, sp) => s + (sp.velocity ?? sumPoints(sp.tasks)), 0) /
		completed.length
	);
}
