 ...
 */
export function roughMessagesTokens(messages) {
 ...
 */
export function roughSystemPromptTokens(systemPrompt) {
...
export function estimateRequestTokens(systemPrompt, messages, tools, maxOutputTokens) {
    const systemPromptTokens = roughSystemPromptTokens(systemPrompt);
    const messagesTokens = roughMessagesTokens(messages);
    const toolsTokens = tools.reduce((sum, tool) => sum + roughToolDefinitionTokens(tool), 0);
