...
function costScore(p: Provider, all: Provider[]): number {
...
function latencyScore(p: Provider, all: Provider[]): number {
...
function qualityScoreFn(p: Provider): number {
  return p.qualityScore / 100;
}

function combinedScore(p: Provider, criteria: SelectionCriteria, all: Provider[]): number {
...
export function rank(providers: Provider[], criteria: SelectionCriteria = "balanced"): Provider[] {
  return providers
...
export function compareCost(a: Provider, b: Provider): number {
...
export function compareLatency(a: Provider, b: Provider): number {
...
export function filterByRegion(providers: Provider[], region: Provider["region"]): Provider[] {
  if (region === "global") return providers;
