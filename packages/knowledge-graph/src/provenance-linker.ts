  ...
  for (const [, nodes] of tagGroups) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        const sharedTags = nodes[i].data.tags.filter(t => nodes[j].data.tags.includes(t));
        if (sharedTags.length >= 3) {
          edges.push({
            ...
            metadata: {
              ...
              confidence: sharedTags.length / Math.max(nodes[i].data.tags.length, nodes[j].data.tags.length),
            },
          });
        }
      }
