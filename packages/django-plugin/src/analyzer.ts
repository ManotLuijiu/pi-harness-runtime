...
async function parseSettings(root: string): Promise<{
	...
	for (const settingsPath of candidates) {
		...
		try {
			...
			const installedMatch = content.match(
				/INSTALLED_APPS\s*=\s*\[([\s\S]*?)\]/m,
			);
			if (installedMatch) {
				const listContent = installedMatch[1];
				// Match quoted strings and identifiers
				const appMatches = listContent.matchAll(
			...
			if (middlewareMatch) {
				...
				const mwMatches = listContent.matchAll(
			...
			if (installedApps.length > 0) break;
		} catch {
			// ignore
		}
	}
...
async function scanDjangoApps(root: string): Promise<DjangoApp[]> {
	...
	for (const searchDir of searchDirs) {
		...
		for (const entry of entries) {
			...
			try {
				isDir = (
					await import("node:fs/promises").then((m) => m.stat(appPath))
				).isDirectory();
			} catch {
				continue;
			}
			...
			if (existsSync(modelsPath)) {
				try {
					...
					const match1 = modelsContent.match(
						/^class\s+\w+\s*\([^)]*models?\.[Mm]odel[^)]*\)/gm,
					);
	...
	for (const cmdDir of searchDirs) {
		try {
			...
			for (const cmd of cmdEntries) {
				if (cmd.startsWith("_") || !cmd.endsWith(".py")) continue;
...
export async function analyzeDjango(
	root: string,
): Promise<DjangoAnalysis | null> {
	if (!isDjangoWorkspace(root)) return null;
	...
	const drfEnabled =
		requirements.some((r) => r.toLowerCase().includes("djangorestframework")) ||
