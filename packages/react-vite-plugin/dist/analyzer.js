...
async function parseAliases(root) {
    ...
    for (const cfgName of [
        ...
        try {
            ...
            const aliasMatches = content.matchAll(/(?:resolve\.alias|alias)\s*[:=]\s*\{([^}]+)\}/g);
            for (const match of aliasMatches) {
                const block = match[1];
                const findMatches = block.matchAll(/(?:find|match)\s*:\s*["']([^"']+)["'][^}]*(?:replacement|replace)\s*:\s*["']([^"']+)["']/g);
                for (const f of findMatches) {
                    aliases[f[1]] = f[2];
                }
                // Simple array alias: ['@', 'src']
                const simple = block.matchAll(/["']([^"']+)["']\s*,\s*["']([^"']+)["']/g);
