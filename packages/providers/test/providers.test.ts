...
describe("MiniMaxAdapter", () => {
  ...
  it("parses MiniMax quota errors", () => {
    const result = adapter.parseError({ message: "Error code: 2056 - Rate limit exceeded" });
    expect(result.quotaExceeded).toBe(true);
    expect(result.rateLimited).toBe(true);
  });

  it("parses rate limit errors", () => {
    const result = adapter.parseError({ message: "Rate limit exceeded, retry after 429" });
    expect(result.rateLimited).toBe(true);
  });
  ...
  it("invokes and returns mock result", async () => {
    const result = await adapter.invoke({ model: "minimax/model-a", messages: [], maxTokens: 100 });
...
describe("OpenAIAdapter", () => {
  ...
  it("parses insufficient_quota error", () => {
    const result = adapter.parseError({ code: "insufficient_quota", message: "Quota exceeded" });
    expect(result.quotaExceeded).toBe(true);
    expect(result.quotaSignal?.exhausted).toBe(true);
  });

  it("parses rate_limit_exceeded error", () => {
    const result = adapter.parseError({ code: "rate_limit_exceeded", message: "Rate limit" });
    expect(result.rateLimited).toBe(true);
  });
...
describe("ClaudeAdapter", () => {
  ...
  it("returns Claude default model", () => {
    expect(adapter.getDefaultModel()).toBe("anthropic/claude-3-5-sonnet-20240620");
  });

  it("returns large token limits for Claude models", () => {
    expect(adapter.getMaxTokens("anthropic/claude-3-5-sonnet-20240620")).toBe(200000);
    expect(adapter.getMaxTokens("anthropic/claude-3-opus-20240229")).toBe(200000);
    expect(adapter.getMaxTokens("anthropic/claude-2")).toBe(100000);
  });

  it("parses rate_limit_error type", () => {
    const result = adapter.parseError({ type: "rate_limit_error", message: "Rate limited" });
    expect(result.rateLimited).toBe(true);
  });

  it("parses overloaded_error type", () => {
    const result = adapter.parseError({ type: "overloaded_error", message: "Overloaded" });
    expect(result.rateLimited).toBe(true);
    expect(result.serverError).toBe(true);
  });

  it("parses quota_error type", () => {
    const result = adapter.parseError({ type: "quota_error", message: "Quota exceeded" });
    expect(result.quotaExceeded).toBe(true);
    expect(result.quotaSignal?.exhausted).toBe(true);
  });

  it("parses authentication_error as client error", () => {
    const result = adapter.parseError({ type: "authentication_error", message: "Auth failed" });
    expect(result.clientError).toBe(true);
  });
});
...
describe("AdapterRegistry", () => {
  ...
  it("overwrites adapter with same id", () => {
    ...
    const a2 = new MiniMaxAdapter({ ...makeConfig("minimax"), name: "New MinMax" });
