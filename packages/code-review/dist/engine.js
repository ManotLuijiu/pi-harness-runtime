...
export class CodeReviewEngine {
     ...
     */
    registerCustomMatcher(name, matcher) {
     ...
     */
    createIssue(rule, file, lineInfo, _matchedCode) {
        return {
     ...
     */
    generateStats(files, issues) {
        const linesReviewed = files.reduce((sum, f) => sum + f.content.split("\n").length, 0);
        return {
