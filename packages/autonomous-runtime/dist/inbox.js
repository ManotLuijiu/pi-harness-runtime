...
export class TaskInbox {
    ...
    append(task) {
        ...
        if (existing) {
            throw new InboxError(`Task ${task.id} already exists in inbox (status=${existing.status})`);
        }
     ...
     */
    transition(taskId, newStatus, extra, event) {
        if (!existsSync(this.tasksPath)) {
    ...
    fail(taskId, reason) {
        ...
        return this.transition(taskId, newStatus, { failureReason: reason, attempts }, {
            ts: new Date().toISOString(),
