 ...
 *
 * Tests:
 * 1. TaskInbox.append, list, get, transition, complete, fail
 * 2. LeaseManager.claim (atomic), heartbeat, release, reap
 * 3. Type consistency
 */
import { describe, it, expect, beforeEach, afterEach } from "bun:test";
import { writeFileSync, unlinkSync, mkdirSync, existsSync, readdirSync, rmSync } from "node:fs";
...
describe("TaskInbox", () => {
	...
	it("append writes one JSONL line", () => {
		...
		const content = require("node:fs").readFileSync(tasksPath(), "utf8");
	...
	it("list returns tasks sorted by priority then createdAt", () => {
		...
		expect(all.map((t) => t.id)).toEqual(["a", "c", "b"]);
	});
	...
	it("list filters by status", () => {
		...
		expect(inbox.list({ status: ["queued", "running"] }).map((t) => t.id).sort()).toEqual(["queued-1", "running-1"].sort());
	});
	...
	it("complete marks status and sets result", () => {
		...
		const result = { taskId: task.id, status: "completed" as const, acceptanceCriteriaMet: [], durationMs: 1000 };
	...
	it("fail increments attempts and sets failureReason", () => {
		const inbox = new TaskInbox({ tasksPath: tasksPath() });
		const task = makeTask();
		inbox.append(task);
...
describe("LeaseManager", () => {
	...
	it("claim returns null for duplicate from same worker (atomic rename fails)", () => {
		const lm = new LeaseManager({ leasesDir: leasesDir() });
		lm.claim("task-1", "worker-1");
		// Same worker trying again → same result (race, one wins)
		const second = lm.claim("task-1", "worker-1");
		// The second claim either wins (same worker) or loses — depends on timing.
		// What matters is it's not two simultaneous writers.
		expect(typeof second?.taskId).toBe("string");
	});
	...
	it("heartbeat extends expiry", async () => {
		...
		expect(new Date(refreshed!.expiresAt).getTime()).toBeGreaterThan(
			new Date(lm.get("task-1")!.expiresAt).getTime() - 5000,
		);
	});
	...
	it("reapAndRelease removes expired leases", async () => {
		...
		expect(lm.get("task-1")).toBeNull();
		expect(lm.get("task-2")).toBeNull();
	});

	it("isExpired returns true for past leases", () => {
