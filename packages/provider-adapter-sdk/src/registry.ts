...
export class AdapterRegistry extends EventEmitter {
	private readonly adapters: Map<string, RegistryEntry> = new Map();
	...
	listByCapability(capability: ProviderCapability): AdapterInfo[] {
		return this.listAdapters().filter((info) =>
	 ...
	 * Invoke an adapter
	 */
	async invoke(id: string, request: ProviderRequest): Promise<AdapterResult> {
		const entry = this.adapters.get(id);
		if (!entry) {
			throw new AdapterNotFoundError(id);
		}
	 ...
	 */
	private checkCompatibility(adapter: BuiltAdapter): CompatibilityResult {
		const issues: string[] = [];
	 ...
	 */
	private createAdapterInfo(entry: RegistryEntry): AdapterInfo {
		return {
			id: entry.adapter.id,
	...
	getStats(): {
		...
		const byState: Record<AdapterState, number> = {
			registered: 0,
			initializing: 0,
			ready: 0,
			error: 0,
		...
		const byCapability: Record<ProviderCapability, number> = {
			code: 0,
			review: 0,
			plan: 0,
			test: 0,
		...
		for (const entry of this.adapters.values()) {
			...
			for (const cap of entry.adapter.getCapabilities()) {
				byCapability[cap as ProviderCapability]++;
			}
		}
