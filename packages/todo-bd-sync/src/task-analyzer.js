 ...
 */
export function analyzeTaskSignals(userMessage) {
    ...
    return {
        ...
        createsFiles: lower.includes("create") ||
            lower.includes("add") ||
            lower.includes("new"),
        explicitlyComplex: COMPLEX_KEYWORDS.some((k) => lower.includes(k)),
        likelySubtasks: SUBTASK_KEYWORDS.some((k) => lower.includes(k)) ||
 ...
 */
export function decideAutoTodo(userMessage, options = {}) {
    ...
    const isTrivial = signals.stepCount === 1 &&
        signals.fileCount <= 1 &&
 ...
 */
export function getTaskComplexityScore(userMessage) {
    const signals = analyzeTaskSignals(userMessage);
    const pendingTasks = getOpenBdIssues().filter((i) => i.status !== "closed");
    let score = 0;
    // Step count (0-25)
