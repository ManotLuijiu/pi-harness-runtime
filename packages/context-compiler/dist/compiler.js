 ...
 */
export async function compileContext(request, clock = createClock()) {
    const { taskId, taskObjective, maximumTokens, candidates, scoringWeights, policy, } = request;
    ...
    if (missingRequired.length > 0) {
        ...
        throw new ContextCompileError("MISSING_REQUIRED_SOURCE", `Required source "${first.source}" has no content`, {
            missingSources: missingRequired.map((c) => c.source),
        });
    }
    // 1. Apply policy filter
    const resolvedPolicy = mergePolicy(policy);
    const { passed: policyPassed, denied: policyDenied } = applyPolicyFilter(candidates, resolvedPolicy);
    const policyOmitted = policyDenied.map(({ candidate, reason }) => ({
        id: candidate.id,
        kind: candidate.kind,
    ...
    const dedupOmitted = dupIds.map((id) => {
    ...
    // 6. Fit to budget
    const { included, omitted: budgetOmitted } = fitToBudget(ranked, maximumTokens);
    ...
    return {
        ...
        estimatedTokens: compiledItems.reduce((sum, item) => sum + item.estimatedTokens, 0),
        sourceGraph,
 ...
 */
function extractDirectReferences(objective) {
    const refs = new Set();
    // Match quoted paths
    ...
    const filePattern = /[a-zA-Z][a-zA-Z0-9_/-]*\.(ts|tsx|js|jsx|py|go|rs|java|cpp|h|c|md|json|yaml|yml|toml)\b/g;
 ...
 */
function extractDependencies(objective) {
    const deps = [];
 ...
 */
function buildSourceGraph(items) {
    const edges = [];
    ...
    for (const okf of okfItems) {
        for (const src of sourceItems) {
            if (okf.content.includes(src.source) ||
    ...
    for (const src of sourceItems) {
        for (const test of testItems) {
            if (test.content.includes(src.source) ||
 ...
 * ```
 */
export function enrichWithOkf(candidates, okfPath) {
