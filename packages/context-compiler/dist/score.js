 ...
 */
export function mergeWeights(user) {
    if (!user)
        return { ...DEFAULT_SCORING_WEIGHTS };
    return {
        priority: user.priority ?? DEFAULT_SCORING_WEIGHTS.priority,
        directFileReference: user.directFileReference ?? DEFAULT_SCORING_WEIGHTS.directFileReference,
        recentFailureRelevance: user.recentFailureRelevance ??
            DEFAULT_SCORING_WEIGHTS.recentFailureRelevance,
        dependencyRelevance: user.dependencyRelevance ?? DEFAULT_SCORING_WEIGHTS.dependencyRelevance,
        frameworkRelevance: user.frameworkRelevance ?? DEFAULT_SCORING_WEIGHTS.frameworkRelevance,
        stalePenalty: user.stalePenalty ?? DEFAULT_SCORING_WEIGHTS.stalePenalty,
        duplicationPenalty: user.duplicationPenalty ?? DEFAULT_SCORING_WEIGHTS.duplicationPenalty,
    };
}
/**
 * Score a candidate.
 ...
 *       - duplication_penalty
 */
export function scoreCandidate(candidate, weights, context) {
    ...
    if (candidate.updatedAt) {
        ...
        score -= Math.min(staleDays * weights.stalePenalty, weights.stalePenalty * 3);
    }
    return score;
}
/**
 ...
 */
export function rankCandidates(candidates, weights, context) {
    return [...candidates]
