...
function reconstructState(
	...
	for (const event of events) {
		if (event.type === "task.started" && event.data?.taskId) {
			currentTaskId = event.data.taskId as string;
		}
		if (event.type === "job.status") {
			status = (event.data?.status as string) || status;
		}
	}
...
async function handleInspect(
	...
	fromSequence?: number,
	toSequence?: number,
): Promise<{ state: JobStateSnapshot; timeline: ReplayEvent[] }> {
	const events = sources.events || [];
	const filteredEvents =
		fromSequence !== undefined || toSequence !== undefined
	...
	for (let i = 0; i < timeline.length; i++) {
		...
		if (event.type.includes("error") || event.type.includes("failure")) {
			divergences.push({
				...
				reason: "Provider output differs" as DivergenceReason,
				severity: "warning",
			});
		}
	}
...
async function handleReexecute(
	...
	if (!validation.valid) {
		throw new Error(
			`Missing required sources: ${validation.missing.join(", ")}`,
		);
	}

	if (!allowExternalCalls) {
		throw new Error(
			"Reexecution with external calls requires explicit approval",
		);
	}
	...
	result.divergences.push({
		sequence: 0,
		reason: "Runtime version differs" as DivergenceReason,
		severity: "info",
	});
...
export class ExperienceReplay {
		...
		if (!validation.valid) {
			return {
				...
				reconstructedState: {
					jobId,
					status: "error",
					checkpoint: sources.checkpoint || ({} as RuntimeCheckpoint),
		...
		try {
			...
			switch (mode) {
				...
				case "simulate":
					...
					break;
				case "reexecute":
					result = await handleReexecute(
						sources,
						jobId,
						request.allowExternalCalls,
					);
					break;
			}
			...
			if (sources.prompts) artifacts.push(...Object.keys(sources.prompts));
			if (sources.outputs) artifacts.push(...Object.keys(sources.outputs));
	...
	private emit(event: ReplayRuntimeEvent): void {
